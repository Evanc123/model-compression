...
# fc1
W_fc1 = weight_variable([28 * 28, size_1])
...
# fc3
W_fc3 = weight_variable([size_2, 10])
...
probs = tf.matmul(h_fc2_drop, fc3_pruned) + b_fc3
y_ = tf.nn.softmax(probs)
...
t_probs = tf.nn.softmax(tf.div(logits, temperature))
y_expert_probs = tf.nn.softmax(tf.div(y_expert, temperature))
...
cross_entropy = (1 - alpha) * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y_, y))
...
loss = cross_entropy + cross_entropy_exper#+ cross_entropy_expert + l2_loss * l2_weight_decay
...
grads_and_vars = optim.compute_gradients(cross_entropy)
l2_loss_grads_and_vars = optim.compute_gradients(l2_loss)
...
if alpha == 0: 
	optimizer = optim.apply_gradients(grads_and_vars )#+ l2_loss_grads_and_vars)
else:
	optimizer = optim.apply_gradients(grads_and_vars + temp_compensated_grads + l2_loss_grads_and_vars)
'''
...
y_train_npy = np.load('x_test_images.npy')
...
def train(iterations=100000, kp1=0.5, kp2=0.5):
	for i in range(iterations):
		...
		x_train_npy, y_train_npy, y_expert_npy, 50)
		if i%100 == 0:
		
			train_accuracy = sess.run(accuracy, feed_dict={
			x:x_batch, 
			y: y_batch, 
			keep_prob1: TEST_KEEP_PROB, 
			keep_prob2: TEST_KEEP_PROB})
			print("step %d, training accuracy %g"%(i, train_accuracy))
		if i%1000 == 0:
			print("test accuracy %g Alpha: %g Temp: %g "% (sess.run(accuracy, feed_dict={
				x: mnist.test.images, 
			y: mnist.test.labels, 
			keep_prob1: TEST_KEEP_PROB, 
			keep_prob2: TEST_KEEP_PROB}), alpha, temperature))
		
		optimizer.run(session=sess, feed_dict={
			x:x_batch,
			y: y_batch, 
			y_expert: y_expert_batch, 
			keep_prob1: kp1,
			 keep_prob2: kp2})
...
if __name__ == '__main__':
	...
	compress(2)  # "We used five iterations of pruning an retraining."
